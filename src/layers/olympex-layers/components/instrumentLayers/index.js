...
class LayerGenerator {
    constructor() {
        this.instruments = ["trackEr2", "trackDc8"]
    ...
    getInstrumentsItem(date, instrumentType, index) {
        ...
        */
        switch (instrumentType) {
            case "trackEr2":
                if (!this.er2_dates.includes(date)) return null; //TODO: later take er2_dates when loaded through addDates.
    ...
    generateLayer() {
        ...
        */
        return this.uniqueDates([...this.dc8_dates, ...this.er2_dates]).map(date => ({
            date,
